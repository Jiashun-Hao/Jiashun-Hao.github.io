---
title: "Java 简单算法——队列"
description: "数据结构中程序基本可以分为两大类：线性结构和非线性结构； - 线性结构代表：数组、队列、链表和栈； - 非线性结构代表：多维数组（包括二维，但是归到这里有些牵强"
...
- ③当front = = rear表示队空；当MaxSize-1 = = rear为队满；  
  ![在这里插入图片描述](./images/8e548b370ed49b2c2b2a21521ff96abd.png)  
  好了，接下来可以写代码了；
...
运行过程来起来也很正常  
![在这里插入图片描述](./images/6a5d1e3ea99263f6734ccaa398a6ad48.png)  
![在这里插入图片描述](./images/0bcc3e856e2590b9e5d3f2e958ec303c.png)
...
![在这里插入图片描述](./images/11bda2f661b8968121629ff540abcb75.png)  
很明显，上面的代码并没有将存入的值真正的取走，即使指向它的指针rear移动了，但数据还是在那里；

或许有人会说：“那在取出的时候加一个删除或置零的操作不就行了吗？”

可以这么做，但是没用；即使这样做了，已经被使用的空间依旧不能再次被利用  
![在这里插入图片描述](./images/96ddd445c181aba00f75451131b30aae.png)  
![在这里插入图片描述](./images/0c5c8a5545fc5d9628405d0913380300.png)
...
- **rear：** 指向队列最后一个**元素的后一个位置**；  
  ![在这里插入图片描述](./images/384b7665d20cb4cc32113596f6c8e425.png)
- **队满的条件：（rear+1）%maxsize == front**；  
  ![在这里插入图片描述](./images/11b8b7d3de0ff551bd0fea8579a7f7dd.png)
- 新建方法：  
  **当前队列中已经使用的空间个数：(rear+maxsize-front)%maxsize;**  
  ![在这里插入图片描述](./images/bd8108864e77bf1beccc4c61e7d87c8c.png)
...
![在这里插入图片描述](./images/c4d2e88a4a1719e3aae654d40f952e4d.png)  
over~