...
![](./images/blog_migrate_e7c6d40b789df490f8480dbb0264352f_png.png)

很明显，父类的引用指向子类的实体，也就是虚拟方法调用，输出的结果是子类重写的方法；

但是，如果我不小心把子类中重写的方法写成`wa1k`呢？（本该是`walk`,这里将字母`l`写成了数字`1`）

![](./images/blog_migrate_f01e19b3c966e317670a19217e411434_png.png)

实际执行的方法就变成了父类的原方法；

由于IDEA的强大，我们在这里可以很清楚的看到提示——子类重写的方法并没有被调用；

![](./images/blog_migrate_81a67632f63ea1c009d7d0a97240073f_png.png)
...
![](./images/blog_migrate_a6522a114b6cd52a2eac709e2d82b7fd_png.png)

**可以看到，当子类的方法确实为父类方法的重写时（两个方法名字一样），没有任何问题。**

**当子类的方法不为父类方法的重写但是又添加了`@Override`时，编译器报错了。**

![](./images/blog_migrate_9e2e78d7a596fc58a564416503d0e369_png.png)

==并且是一个致命的错误；==

![](./images/blog_migrate_70827cebeb64a2c8572cd855b4d7ed78_png.png)
...
![](./images/blog_migrate_d35a85945a47a5b7c91eeef555ad0735_png.png)

没错，`@Deprecated`的出现就意味着它标志的程序元素像房子一样被写了一个“拆”字；该程序元素（类、方法等）已经或者将要被淘汰，会有新的程序元素来实现该程序元素的功能。

**但是，被标记的程序并非不能使用，`@Deprecated`并没有强制性，它只是一种标记**![](./images/blog_migrate_5007de19287bc2b3c1fbbf8c8c3df8c1_png.png)
...
![](./images/blog_migrate_99129ba76cdf9e672e160aa5bcebb1ed_png.png)

这时候，我们就可以使用`@SuppressWarnings`

![](./images/blog_migrate_e627071d5abdfbe759321687f973b9ed_png.png)
...
![](./images/blog_migrate_78ca435fe1cce725f6c6cc02fb5276fc_png.png)
<br>
...
![](./images/blog_migrate_9596083652825b80a26a74c6621ee79b_png.png)
...
![](./images/blog_migrate_1c410e35e52078540739ac10a7c4b9f7_png.png)

可以看到，除去不影响程序注释（注意不是注解），`@SuppressWarnings`的源码格式和我们自定义的几乎一样，唯一的区别就是在于上面的两行代码；

![](./images/blog_migrate_ee2f35a6715b7e90fe0b684c4a92f9e4_png.png)
...
![](./images/blog_migrate_c80969396ec79649c257e94ad577a2c3_png.png)

也可以这样（注意看包）：

![](./images/blog_migrate_1da43f536c75980e16f0ff1676820a16_png.png)
...
![](./images/blog_migrate_76bc1448b322fa89fb8fe0d5649f2ee4_png.png)
...
![](./images/blog_migrate_4623bb425bae8e6bc85ffed5aa0b8cdb_png.png)

<br>
### 4.@Inherited
作用：此注解基于继承，**被`@Inherited`所修饰的父类其子类可以继承父类中的注解；**

![](./images/blog_migrate_0a48a30c4c9d75d0f691d697d80ef0c0_png.png)

![](./images/blog_migrate_91c5ea918313c45301408a33e14e7c68_png.png)
