---
title: "Java简单笔记——泛型"
description: "- [1.什么是泛型？](- [2.为什么要有泛型？](- [3.泛型的使用](- [4.自定义泛型类](- [5.泛型类的继承](- [6.泛型和继承的关系]"
...
在Java中，所有操作的类的都可以视为Object类的子类。对于集合来说，添加、删除、修改等操作时不考虑类型。但是很多的时候，我们需要程序只能处理我们规定的类型。  
 ![在这里插入图片描述](./images/ec9887125590522ed8315453cc4cc240.png)  
...
![在这里插入图片描述](./images/fec11e2a2c4e15b0bceffa831e915fa5.png)  
...
![在这里插入图片描述](./images/775b0658b53eeac9e4e155d688b26035.png)  
 我们可以看到，当没有进行数据转换的时候，无论什么类型都可以添加进该集合，要解释这一点也并不难，因为集合中操作的就是Object类。  
 ![在这里插入图片描述](./images/f605c0622a961e8e7404773a65bc1190.png)  
...
![在这里插入图片描述](./images/da23e954204396d992c56fe2b87deb49.png)  
 amazing！  
...
当我们这样写了以后，其实就规定了该集合中只能存放`Integer`型数据；  
 ![在这里插入图片描述](./images/5896e0c4f70de31453e12deabe105be1.png)  
 ![在这里插入图片描述](./images/b999ea3fed14684e5e87b8b2402c687d.png)  
 好了，问题解决了。但是，为什么我们可以在集合中使用泛型呢？

好办，查看一下源码：  
 ![在这里插入图片描述](./images/210e47674cf353054afa782e61d05d5b.png)  
 同样的，我们看一下**Map**集合：  
 ![在这里插入图片描述](./images/7220f9f0c6b2362ce8a47f03a4bef322.png)  
 但是Map集合比较特殊，他的定义是这样的；  
 ![在这里插入图片描述](./images/f6d4db26fc2b3d5ca07719e06ebb879b.png)  
 ![在这里插入图片描述](./images/9b3b7a090e02860b76ed588faabc98e6.png)
...
这里没有指定泛型，所以默认的类型为`Object`  
 ![在这里插入图片描述](./images/c2dd83a203bab72260a5bee7757d62a9.png)
...
一切正常：  
 ![在这里插入图片描述](./images/8d2ecd1d2f938d44c706c2ce51d9f4ac.png)

## 5.泛型类的继承

关于继承，有个很重要的特点就是子类的实例可以赋值给父类的引用：  
 ![在这里插入图片描述](./images/1436e6f8b0af85a3c92fa608630331d8.png)  
 泛型类的继承可以分成两种，一种是先指明泛型的类型：  
 ![在这里插入图片描述](./images/049986fb1516ba8529de12bcf4f2b2b1.png)  
 一种是不指明类型：  
 ![在这里插入图片描述](./images/0b4f5e66c4ac6b402ba71adcc416ca70.png)  
 效果和刚才一样，对于指定类型的子类，只能处理所指定的类型；而对于没有指定类型的子类，类型还是`Object`  
 ![在这里插入图片描述](./images/1279210ddae2c2fdb3f087af7288270a.png)  
 ![在这里插入图片描述](./images/4cf542da1ad0cb14e492a374f5ae982f.png)

## 6.泛型和继承的关系

我们都知道，根据Java中多态的特性，子类的对象可以赋值给父类的引用，  
 例如：  
 ![在这里插入图片描述](./images/bfeb1b20967482fc718da95db7c1f0f1.png)  
 但是在泛型中，子类和父类的关系却不是这样；  
 为了方便测试，我们实例化两个`list`的对象；  
 将其中一个的泛型定义为object，另一个定义为String  
 ![在这里插入图片描述](./images/32eabb75a8a8ff31f37a3dc0dbe29bd4.png)  
 然后，将`list2`赋值给`list1`；  
 ![在这里插入图片描述](./images/dc1d03766fe2ccd74966aa2adfd54ccd.png)  
...
例如，如果泛型直接存在着这种可以赋值的关系，那么如下的操作便可以实现；![在这里插入图片描述](./images/bae600287991c75b214d90f523873db5.png)  
 因为可以实现，那么此时的`list1`与`list2`便操作的是同一片内存空间；  
 但是又因为两个对象都具有`list`的方法，所以便可这样：  
 ![在这里插入图片描述](./images/eeced2fb4fb40b3666f1c8afb9931860.png)  
 这里便出现了问题；  
...
这个非常简单，先说结论：**List <?>是所有List泛型的父类**  
 这其中的<?>，就是通配符  
 测试：  
 ![在这里插入图片描述](./images/651d2f372e01f93a04e9eddb3b959f0c.png)  
 通配符的作用一般用于写一些公共的方法：  
 ![在这里插入图片描述](./images/8a9e3d544e3a007511ca17db4f6acea7.png)  
...
可以看到，泛型方法的原始类型不是`Object`  
 ![在这里插入图片描述](./images/e418a11ad8deac6f290250aea8345d25.png)当通过对象调用泛型方法，可以直接使用数据指明类型，并且是一次性的。  
 输出结果：  
 ![在这里插入图片描述](./images/98ea951f709f79b6537a4247fdf60903.png)  
 有什么用？  
 举个小实例：**实现集合到数组的复制**：
...
结果：  
 ![在这里插入图片描述](./images/26e934f97e0aa4b265c354c512c1ab50.png)  
...
读取操作可以运行：  
 ![在这里插入图片描述](./images/4228820f1b15523dec1392fd23bd94f5.png)  
 但是在写入的时候；  
 ![](./images/0b51dda4a232dc52b399824dec48dbbd.png)  
 写什么？写不了？也不是，可以存一个值；  
 ![在这里插入图片描述](./images/549b0b3ad1aa77c1602a1e8e47520bfc.png)  
 还有没？没了~