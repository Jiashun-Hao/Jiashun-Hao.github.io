...
source: "https://blog.csdn.net/HJS1453100406/article/details/104418764"
tags:
  - '未分类'
description: '序言：首先说明一点，其实本篇主要是叙述面向对象三要素之一的“多态”，但是我发现我好像没有专门写和“封装”有关的博客，在加上我目前理解的层次水平不是很高，才疏学浅的我觉得“封装”相对于“继承”和“多态”...'
language: 'Chinese'
---
...
JAVA类的封装就是指隐藏对象的属性和实现细节,仅对外提供公共访问方式。 主要好处是: 将变化隔离。 便于使用 提高重要性。 提高安全性。  
 ...
 利用关键字，将一个类中的属性和方法设置为只能被当前所在该类中的**方法**使用和修改，若其它的类想调用和使用该类中的属性和方法，只能通过该类中开放的方法所调用  
 还是有点蒙？Don’t worry!  
...
嗯，按理说这时候`xiaoming` 这个对象被实例化了，可以使用`对象.属性/方法`使用，但是，真的是这样吗？  
 ...
 ![在这里插入图片描述](images/blog_migrate_702ed457b2403cb90ebc040112c859b9_png.png)  
...
输出结果：  
 ![在这里插入图片描述](images/blog_migrate_3e0a11930b49e6a28629fbdd764b10f3_png.png)  
 over~~
...
1.多态是指一个事物的多种形态，它的构建基于继承和重写；  
 2.虚拟方法调用：通过父类的引用指向子类的实体，从而调用在子类中被子类所重写的父类的方法  
 3.程序运行时候分为两部分：编译状态和运行状态；对于多态来说，编译时候看等号前面，运行时候看等号后面
...
我们试着调出`xiao`的属性和方法，看看它到底具有那些属性和方法；  
 如图：  
 ![在这里插入图片描述](images/blog_migrate_56a8857dbb167a9b5b331c2313b1bd5c_png.png)  
...
输出结果：  
 ![在这里插入图片描述](images/blog_migrate_ffc79fb850aac33bcc95c6372b0ae74e_png.png)  
...
使用第二行代码以后。我们可以看到，对象`hua`具有了子类的全部属性，包括’id’;  
 ![在这里插入图片描述](images/blog_migrate_51f46b18350db3595a3c698086a07fa7_png.png)  
 **2.必要的几点说明：**  
 1.同为一个父类多个子类，不能相互转换；  
...
结果：  
 ![在这里插入图片描述](images/blog_migrate_93f61edb85a7fab223a7428bec9f94d9_png.png)  
...
结果：  
 ![在这里插入图片描述](images/blog_migrate_70a338a71be59dd0bb4567e1e481b471_png.png)  
 over~~
