...
description: "- ](    - [7.1.4 Leaky ReLU 函数](  - [7.2 后向传播的代码](- [八、梯度下降](- [九、构建预测](- [十、聚合和"
...
放此图在这里为了快速认识，也为了需要的时候快速查找。初次看的时候看不懂没关系，可以先看下面的内容。  
 ![在这里插入图片描述](./images/b347bff8dcae19e9ebef1b28279ceb28.png)
...
Y^的关系  
 那么我们现在知道的神经网络结构这样的：  
 ![在这里插入图片描述](./images/afd05889f0fb91aa9b50144ade6cf477.png)  
 总的来说，就是将特征输入到**线性回归+激活函数**函数中，得到预测，然后计算损失，然后调整模型参数，然后重复这一过程。

现在，我们添加一个东西，顺便完善一下这个结构  
 ![在这里插入图片描述](./images/5173de024230545ac55896096165f42d.png)

现在，在中间添加了一个层，名字叫**隐藏层**。

从目前来看，这个层中的元素个数有四个，分别从
a
[
1
]
...
4
a[1]^4
a[1]4，而这**四个节点**呢，每一个其实都是一个**线性回归+激活函数**（**线性回归+激活函数**也就是**逻辑回归**）

我们可以看到，输入层中的每一个元素都于这四个元素进行了全链接，如果换个角度来看的话，就是：

在隐藏层中，输入的样本被每一个**逻辑回归**单元进行了一次**预测**

然后，我们将隐藏层中得到的预测再次进行拟合，也就是传入到输出层，得到一个最终的
y
^
\hat y
y^​
...
b的不止一个。  
 我们再看一下这张图：  
 ![在这里插入图片描述](./images/8d118a943e8a61b2debde8fecbe04744.png)
...
转化线性函数的为非线性函数，使得模型具有更多的功能。
转化线性函数的为非线性函数，使得模型具有更多的功能。
转化线性函数的为非线性函数，使得模型具有更多的功能。  
 对了，还有一句话，别问为什么，记住就行————

除了用于二分类任务最后的输出，否则不要用
s
i
g
m
...
关于tanh函数，它的图像是这样的：上趋近于1,下趋近于-1  
 ![在这里插入图片描述](./images/935742495ccb13d6507e807ccdccdfa0.png)  
 由于
t
a
n
h
tanh
tanh函数的输出值位于-1到1之间。这意味着它会将输入值归一化到这个范围内，所以如果我们想控制和规范神经元的输出值，我们可以选择这个函数。

在这道题中，为了练习。对于第一个激活函数，我选择使用了
t
a
n
h
tanh
tanh函数，而之后涉及到最终输出的激活函数，还是需要
s
i
g
m
...
Z^{\left[2\right]}
Z[2]经过 非线性函数变换 以后的 最终输出，也就是预测结果
y
^
\hat y
y^​
...
测试结果：  
 ![在这里插入图片描述](./images/7d0026d2bec6ae1f8669a26a302a3066.png)
...
J(w^{\left[1\right]}, b^{\left[1\right]},w^{\left[2\right]}, b^{\left[2\right]}) = \frac{1}{m} \sum\_{i=1}^{m} \text{L}(y^{(i)}, \hat{y}^{(i)})
J(w[1],b[1],w[2],b[2])=m1​i=1∑m​L(y(i),y^​(i))
...
J(w^{\left[1\right]}, b^{\left[1\right]},w^{\left[2\right]}, b^{\left[2\right]}) = \frac{1}{m} \sum\_{i=1}^{m} \text{L}(y^{(i)}, A^{\left[2](i\right)})
J(w[1],b[1],w[2],b[2])=m1​i=1∑m​L(y(i),A[2](i))  
 **在深度学习中，我们需要找合适的
w
w
w和
...
测试结果：  
 ![在这里插入图片描述](./images/6e692db90c496088aab7527131f5f611.png)
...
db[2]=m1​np.sum(dz[2],axis=1,keepdims=True)  
...
db[1]=m1​np.sum(dz[1],axis=1,keepdims=True)
...
测试结果：  
 ![在这里插入图片描述](./images/45044c560ebbf24cbfb9566eedeaa065.png)
...
测试结果：  
 ![在这里插入图片描述](./images/028506b3cdd5be2c1ae96b71d3682520.png)
...
测试结果：  
 ![在这里插入图片描述](./images/83ad2a71b7a4c0cda9486f2258164f2f.png)
...
![在这里插入图片描述](./images/8f90e2e3f9a27b7a48e5d42ced2891c3.png)  
 ![在这里插入图片描述](./images/8557c7abef95e531730375bc16fb0b68.png)
