...
- [三、初始化函数](#_305)
- [四、激活函数](#_344)
- - [4.1 tanh（双曲正切函数）函数](#41_tanh_356)
- [五，前向传播](#_377)
- [六、损失函数](#_446)
- [七、后向传播](#_496)
- - [7.1 激活函数的导数](#71__513)
  - - [7.1.1 Sigmoid 函数](#711_Sigmoid__515)
    - [7.1.2 Tanh 函数](#712_Tanh__518)
    - [7.1.3 ReLU 函数 (Rectified Linear Unit)](#713_ReLU__Rectified_Linear_Unit_521)
    - [7.1.4 Leaky ReLU 函数](#714_Leaky_ReLU__531)
  - [7.2 后向传播的代码](#72__550)
- [八、梯度下降](#_598)
- [九、构建预测](#_645)
- [十、聚合和主函数](#_669)
...
放此图在这里为了快速认识，也为了需要的时候快速查找。初次看的时候看不懂没关系，可以先看下面的内容。  
 ![在这里插入图片描述](images/blog_migrate_b347bff8dcae19e9ebef1b28279ceb28_png.png)
...
首先，我们要引入一个新的中间层，**隐藏层。**  
 让我们先不考虑损失的计算，也就是现忽略标签FORMULA\_PLACEHOLDER\_0\_END数据集合，单单来讨论FORMULA\_PLACEHOLDER\_5\_END和FORMULA\_PLACEHOLDER\_10\_END的关系  
 那么我们现在知道的神经网络结构这样的：  
 ![在这里插入图片描述](images/blog_migrate_afd05889f0fb91aa9b50144ade6cf477_png.png)  
 总的来说，就是将特征输入到**线性回归+激活函数**函数中，得到预测，然后计算损失，然后调整模型参数，然后重复这一过程。

现在，我们添加一个东西，顺便完善一下这个结构  
 ![在这里插入图片描述](images/blog_migrate_5173de024230545ac55896096165f42d_png.png)

现在，在中间添加了一个层，名字叫**隐藏层**。

从目前来看，这个层中的元素个数有四个，分别从FORMULA\_PLACEHOLDER\_15\_END 到FORMULA\_PLACEHOLDER\_20\_END，而这**四个节点**呢，每一个其实都是一个**线性回归+激活函数**（**线性回归+激活函数**也就是**逻辑回归**）

我们可以看到，输入层中的每一个元素都于这四个元素进行了全链接，如果换个角度来看的话，就是：

在隐藏层中，输入的样本被每一个**逻辑回归**单元进行了一次**预测**

然后，我们将隐藏层中得到的预测再次进行拟合，也就是传入到输出层，得到一个最终的FORMULA\_PLACEHOLDER\_25\_END
...
现在，我们可以来构建初始化函数了，也就是对FORMULA\_PLACEHOLDER\_30\_END和FORMULA\_PLACEHOLDER\_35\_END赋值了。这里有两个关键点：  
 1：初始化最好用随机  
 2：需要构建的FORMULA\_PLACEHOLDER\_40\_END和FORMULA\_PLACEHOLDER\_45\_END的个数。

第一个我在第一章的时候解释过，随机的FORMULA\_PLACEHOLDER\_50\_END比全0的要好。  
 但是第二个，为什么要构建FORMULA\_PLACEHOLDER\_55\_END和FORMULA\_PLACEHOLDER\_60\_END的个数呢？因为这个时候的FORMULA\_PLACEHOLDER\_65\_END和FORMULA\_PLACEHOLDER\_70\_END的不止一个。  
 我们再看一下这张图：  
 ![在这里插入图片描述](images/blog_migrate_8d118a943e8a61b2debde8fecbe04744_png.png)

在神经网络中，每一层对应需要FORMULA\_PLACEHOLDER\_75\_END和FORMULA\_PLACEHOLDER\_80\_END都不一样。

当前有两个层：输入层和隐藏层会用到FORMULA\_PLACEHOLDER\_85\_END和FORMULA\_PLACEHOLDER\_90\_END，所以就定义两组。
...
激活函数是做什么的呢？  
 用一句话来说就是：  
 FORMULA\_PLACEHOLDER\_95\_END  
 对了，还有一句话，别问为什么，记住就行————

FORMULA\_PLACEHOLDER\_100\_END

不用FORMULA\_PLACEHOLDER\_112\_END用什么呢？

让我来介绍另外一种激活函数: **FORMULA\_PLACEHOLDER\_123\_END函数**

### 4.1 tanh（双曲正切函数）函数

关于tanh函数，它的图像是这样的：上趋近于1,下趋近于-1  
 ![在这里插入图片描述](images/blog_migrate_935742495ccb13d6507e807ccdccdfa0_png.png)  
 由于FORMULA\_PLACEHOLDER\_129\_END函数的输出值位于-1到1之间。这意味着它会将输入值归一化到这个范围内，所以如果我们想控制和规范神经元的输出值，我们可以选择这个函数。

在这道题中，为了练习。对于第一个激活函数，我选择使用了FORMULA\_PLACEHOLDER\_135\_END函数，而之后涉及到最终输出的激活函数，还是需要FORMULA\_PLACEHOLDER\_141\_END函数
...
假设：FORMULA\_PLACEHOLDER\_152\_END =（FORMULA\_PLACEHOLDER\_158\_END，FORMULA\_PLACEHOLDER\_163\_END，FORMULA\_PLACEHOLDER\_168\_END）  
 FORMULA\_PLACEHOLDER\_173\_END：输入特征的个数；  
 FORMULA\_PLACEHOLDER\_178\_END：隐藏单元的个数；  
 FORMULA\_PLACEHOLDER\_183\_END：输出特征的个数；

此外，给定单层神经网络的逻辑回归方程：

**隐藏层：**  
 FORMULA\_PLACEHOLDER\_188\_END  
 FORMULA\_PLACEHOLDER\_197\_END

// 其中FORMULA\_PLACEHOLDER\_205\_END是输入的线性函数的输出，FORMULA\_PLACEHOLDER\_210\_END是FORMULA\_PLACEHOLDER\_215\_END进过 **非线性函数变换 **以后的** 输出**

FORMULA\_PLACEHOLDER\_220\_END的维度为（FORMULA\_PLACEHOLDER\_225\_END，FORMULA\_PLACEHOLDER\_230\_END）  
 FORMULA\_PLACEHOLDER\_235\_END的维度为（FORMULA\_PLACEHOLDER\_240\_END，FORMULA\_PLACEHOLDER\_245\_END）

**输出层：**  
 FORMULA\_PLACEHOLDER\_249\_END  
 FORMULA\_PLACEHOLDER\_258\_END

FORMULA\_PLACEHOLDER\_266\_END的维度为（FORMULA\_PLACEHOLDER\_271\_END，FORMULA\_PLACEHOLDER\_276\_END）  
 FORMULA\_PLACEHOLDER\_281\_END的维度为（FORMULA\_PLACEHOLDER\_286\_END，FORMULA\_PLACEHOLDER\_291\_END）

// 其中FORMULA\_PLACEHOLDER\_295\_END是 **非线性输出FORMULA\_PLACEHOLDER\_300\_END** 添加上当前的 **权重FORMULA\_PLACEHOLDER\_305\_END和偏差FORMULA\_PLACEHOLDER\_310\_END** 所构成的线性输出。

FORMULA\_PLACEHOLDER\_315\_END是FORMULA\_PLACEHOLDER\_320\_END经过 非线性函数变换 以后的 最终输出，也就是预测结果FORMULA\_PLACEHOLDER\_325\_END
...
测试结果：  
 ![在这里插入图片描述](images/blog_migrate_7d0026d2bec6ae1f8669a26a302a3066_png.png)
...
FORMULA\_PLACEHOLDER\_344\_END

**或者也可以使用 FORMULA\_PLACEHOLDER\_361\_END 代替FORMULA\_PLACEHOLDER\_366\_END**

FORMULA\_PLACEHOLDER\_372\_END  
 **在深度学习中，我们需要找合适的FORMULA\_PLACEHOLDER\_389\_END和FORMULA\_PLACEHOLDER\_394\_END，尽可能的缩小FORMULA\_PLACEHOLDER\_399\_END**
...
测试结果：  
 ![在这里插入图片描述](images/blog_migrate_6e692db90c496088aab7527131f5f611_png.png)

## 七、后向传播

所谓后向传播，就是计算FORMULA\_PLACEHOLDER\_406\_END和FORMULA\_PLACEHOLDER\_411\_END需要改变多少，只是计算，不改变。改变是梯度下降的事情。
...
**后向传播公式(其中的Y是(1,m)的标签集)：**  
 FORMULA\_PLACEHOLDER\_416\_END  
 FORMULA\_PLACEHOLDER\_425\_END  
 FORMULA\_PLACEHOLDER\_437\_END  
 FORMULA\_PLACEHOLDER\_466\_END  
 FORMULA\_PLACEHOLDER\_479\_END  
 FORMULA\_PLACEHOLDER\_491\_END

### 7.1 激活函数的导数

激活函数的导数，用符号表示：FORMULA\_PLACEHOLDER\_520\_END

#### 7.1.1 Sigmoid 函数

Sigmoid 函数: FORMULA\_PLACEHOLDER\_526\_END  
 导数: FORMULA\_PLACEHOLDER\_535\_END

#### 7.1.2 Tanh 函数

Tanh 函数: FORMULA\_PLACEHOLDER\_546\_END  
 导数: FORMULA\_PLACEHOLDER\_560\_END

#### 7.1.3 ReLU 函数 (Rectified Linear Unit)

ReLU 函数: FORMULA\_PLACEHOLDER\_567\_END  
 导数:  
 FORMULA\_PLACEHOLDER\_577\_END

#### 7.1.4 Leaky ReLU 函数

Leaky ReLU 函数导数:  
 FORMULA\_PLACEHOLDER\_589\_END

Leaky ReLU 函数导数:  
 FORMULA\_PLACEHOLDER\_598\_END
...
测试结果：  
 ![在这里插入图片描述](images/blog_migrate_45044c560ebbf24cbfb9566eedeaa065_png.png)
...
更新FORMULA\_PLACEHOLDER\_607\_END和FORMULA\_PLACEHOLDER\_612\_END，其中 FORMULA\_PLACEHOLDER\_617\_END是学习率

公式：  
 FORMULA\_PLACEHOLDER\_622\_END  
 FORMULA\_PLACEHOLDER\_632\_END
...
测试结果：  
 ![在这里插入图片描述](images/blog_migrate_028506b3cdd5be2c1ae96b71d3682520_png.png)

## 九、构建预测

这一步很关键，我们要使用FORMULA\_PLACEHOLDER\_642\_END来生成预测，与它们的标签FORMULA\_PLACEHOLDER\_647\_END来进行拟合。
...
测试结果：  
 ![在这里插入图片描述](images/blog_migrate_83ad2a71b7a4c0cda9486f2258164f2f_png.png)
...
![在这里插入图片描述](images/blog_migrate_8f90e2e3f9a27b7a48e5d42ced2891c3_png.png)  
 ![在这里插入图片描述](images/blog_migrate_8557c7abef95e531730375bc16fb0b68_png.png)
