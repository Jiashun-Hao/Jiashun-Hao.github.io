...
- [二、数据的预处理](#_63)
- - [2.1 数据集说明](#21__64)
  - [2.2 数据集降维度并且转置](#22__159)
  - [2.3 数据预处理完整代码](#23__209)
- [三、逻辑回归概念以及函数](#_236)
- - [3.1 线性回归函数公式](#31__252)
  - [3.2 sigmoid函数](#32_sigmoid_264)
- [四、初始化函数](#_310)
- [补充：链式法则与导数](#_346)
- - [链式法则的定义](#_353)
  - [在深度学习中的应用](#_362)
...
- - [5.1 损失函数](#51__405)
  - [5.2 前向传播](#52__463)
  - [5.3 后向传播](#53__501)
  - - [5.3.1 梯度下降算法](#531__507)
  - [5.4 前向传播和后向传播完整代码](#54__545)
  - - [5.4.1 :关于dz的说明](#541_dz_599)
- [六、模型使用](#_613)
- - [6.1 运行梯度下降更新w和b](#61_wb_618)
  - [6.2 模型使用：构建预测函数（计算数据转化的值）](#62__678)
  - [6.3 聚合](#63__718)
- [七、完整代码](#_758)
- [再玩一下？好的！](#_1130)
- [写在后面的话](#_1165)
...
放此图在这里为了快速认识，也为了需要的时候快速查找。初次看的时候看不懂没关系，可以先看下面的内容。  
 ![在这里插入图片描述](images/blog_migrate_1ee86f1a2efdff2c36e7e48c935b76b2_png.png)
...
在给出的文件lr\_utils 中的load\_dataset函数是对训练集和测试集的预处理，并且已经封装好了
...
其实就是保存的209个，大小为64\*64像素的RGB的图片

**举个例子：**  
 ![在这里插入图片描述](images/blog_migrate_0f570d28852474ad1e590a7e0f2f83e6_png.png)
...
![在这里插入图片描述](images/blog_migrate_10629ef708e6da9268d8aba0deb13c3b_png.png)  
 这三个底色图片的参数都不一样，因为不一样，所以才可以构成千颜万色。

对于当前的步骤，我们不需要关心每一个矩阵中的参数是怎么样的。

我们只需要知道，一个图片，在计算机中完整的**像素的个数**是：高像素 X 宽像素 X 3  
 ![在这里插入图片描述](images/blog_migrate_23dabf6793e16a3f3076ad70d1827afb_png.png)

所以，一个图片的矩阵向量的表现形式就是 **[ 高像素 , 宽像素 , 3 ]** ，这条猫猫的矩阵向量的表现形式是[5,5,3]

![在这里插入图片描述](images/blog_migrate_8a693d140c07b3a06a663f17045700d3_png.png)  
...
![在这里插入图片描述](images/blog_migrate_1cbd39e759001444240063c9267c3da9_png.png)
...
![在这里插入图片描述](images/blog_migrate_4329a5543ba4d75d29fb62ee6c502c37_png.png)
...
![在这里插入图片描述](images/blog_migrate_ec9298ba9c88e8836bfa6b24de76ad66_png.png)

此外，我们刚刚提到的,测试了两个数据集合的维度,可以看到它们是不匹配的：  
 根据矩阵先行数后列数，我们可以看到样本是一个 `209` X`64`X`64`X`3`的四维矩阵，而标签则是一个`1`行`209`列的二维矩阵。
...
所以，为了方便数据输入到模型，我们希望统一训练集和测试集的格式  
...
**当然，因为`reshape` 函数允许我们重新组织数组的维度，但它 **不会** 更改数组元素的顺序或值。**
...
**具体到实际功能，逻辑回归的目标是是找到一个决策边界，然后根据样本点相对于这个边界的位置来分类，对于线性逻辑回归，这个决策边界是一条直线（在二维空间中）或一个超平面（在更高的维度中），这取决于特征的数量。**

就是说，找一条线，将数据划分为两个部分。  
 ![在这里插入图片描述](images/blog_migrate_fb94aafe78d02b602bc71492505bcbaf_png.png)

如果维度高的话，是找一个面来划分  
 ![在这里插入图片描述](images/blog_migrate_a225e8725234cf464766bd92c5d40493_png.png)  
 对于逻辑回归的函数，我们可以理解为：

**逻辑回归函数=线性回归函数 + sigmoid函数**

### 3.1 线性回归函数公式

FORMULA\_PLACEHOLDER\_0\_END

1. FORMULA\_PLACEHOLDER\_13\_END: 第i个样本的特征向量
2. w: 权重向量，表示当前FORMULA\_PLACEHOLDER\_19\_END的权重比（权重用于决定决策边界怎么画）
...
4. b: 这是偏置项，它是一个常数。
5. FORMULA\_PLACEHOLDER\_25\_END: 第i个样本的线性输出。输入一个FORMULA\_PLACEHOLDER\_31\_END，会得到一个 FORMULA\_PLACEHOLDER\_37\_END
...
公式如下：  
 FORMULA\_PLACEHOLDER\_43\_END  
 补充：“z” 就是经过线性函数计算以后得到的结果，在代码编写中直接向sigmoid传入线性函数和参数

嵌套线性回归函数公式：

FORMULA\_PLACEHOLDER\_52\_END  
 关于sigmoid函数，我们需要知道：
...
当我们将一个FORMULA\_PLACEHOLDER\_65\_END输入到sigmoid函数中，会得到一个 FORMULA\_PLACEHOLDER\_71\_END或者 FORMULA\_PLACEHOLDER\_76\_END ,它是由 FORMULA\_PLACEHOLDER\_82\_END生成的和当前标签 FORMULA\_PLACEHOLDER\_88\_END同类型的值（这也是为什么用hat/y表示而不是用别的字母）

它是0和1之间的概率值，表示正类（也就是1）的预测概率。

FORMULA\_PLACEHOLDER\_94\_END

FORMULA\_PLACEHOLDER\_104\_END

为了得到一个明确的分类预测（0或1），可以设定一个阈值，通常为0.5。如果 FORMULA\_PLACEHOLDER\_116\_END大于0.5，则预测为1，否则为0。

**所以，在这个公式中，为了让FORMULA\_PLACEHOLDER\_121\_END 趋近于1，我们便需要让 FORMULA\_PLACEHOLDER\_126\_END的值尽可能的大。**

**所以，逻辑回归的目标，就是找到合适的 FORMULA\_PLACEHOLDER\_135\_END和 FORMULA\_PLACEHOLDER\_140\_END**
...
下一步，既然要找 FORMULA\_PLACEHOLDER\_145\_END和 FORMULA\_PLACEHOLDER\_150\_END，我们就应该写一个初始化函数先给 FORMULA\_PLACEHOLDER\_155\_END和 FORMULA\_PLACEHOLDER\_160\_END 赋默认值。
...
链式法则告诉我们，如果你想要计算复合函数 h 相对于 x 的导数，你可以将 f 相对于 g 的导数乘以 g 相对于 x 的导数。用数学术语来表示就是：  
 FORMULA\_PLACEHOLDER\_165\_END
...
然后，使用链式法则，我们可以一层层向后计算，直到达到`输入层`，从而得到损失函数相对于所有权重的梯度。
...
**假设：** 考虑一个简单的神经网络层，其输出 y 由输入 x、权重 w 和偏置 b 通过函数 y = f(wx + b) 计算得出。在这里，f 可能是一个激活函数，如ReLU或Sigmoid。如果我们想要计算损失 L 相对于 w 的梯度，我们就会使用链式法则：  
 FORMULA\_PLACEHOLDER\_181\_END

其中，FORMULA\_PLACEHOLDER\_198\_END 是损失相对于层输出的梯度，而 FORMULA\_PLACEHOLDER\_206\_END 是层输出相对于权重的梯度。通过链式法则，我们可以将这两个梯度相乘，从而得到我们需要的梯度 FORMULA\_PLACEHOLDER\_214\_END。这种方法可以用来对网络中的所有权重进行类似的计算。
...
1. 前向传播：给当前数据FORMULA\_PLACEHOLDER\_222\_END，根据算法（当前我们用的是sigmoid函数）输出一个结果FORMULA\_PLACEHOLDER\_228\_END
2. 计算机损失：得到前向传播的预测FORMULA\_PLACEHOLDER\_234\_END以后，我们会评估“预测与我们给定的标签FORMULA\_PLACEHOLDER\_240\_END之间的差异。这个差异（通常称为“损失”或“误差”）
...
补充：下面这些废话当个乐看看就行，核心知识点：  
 这个函数的作用为，将FORMULA\_PLACEHOLDER\_246\_END和FORMULA\_PLACEHOLDER\_252\_END输入到损失函数中：  
...
有没有什么办法可以让标签 (FORMULA\_PLACEHOLDER\_258\_END) 为 **1** 的 FORMULA\_PLACEHOLDER\_264\_END 的预测值 (FORMULA\_PLACEHOLDER\_270\_END) 趋近于 **1**？

有没有什么办法可以让标签 (FORMULA\_PLACEHOLDER\_276\_END) 为 **0** 的 FORMULA\_PLACEHOLDER\_282\_END 的预测值 (FORMULA\_PLACEHOLDER\_288\_END) 趋近于 **0**？

这个时候，有一个拿到 **两！根！筷！子！** 的哲学家说，简单啊，你让

可以让标签 (FORMULA\_PLACEHOLDER\_294\_END) 为 **1** 的 FORMULA\_PLACEHOLDER\_300\_END 的预测值 (FORMULA\_PLACEHOLDER\_306\_END) 不趋近于 **0**  
 可以让标签 (FORMULA\_PLACEHOLDER\_312\_END) 为 **0** 的 FORMULA\_PLACEHOLDER\_318\_END 的预测值 (FORMULA\_PLACEHOLDER\_324\_END) 不趋近于 **1**

在把这个哲学家祭天以后，一个数学家仔细琢磨这句话，说：“不如这样吧，我们可以想一个公式，在公式里面同时输入FORMULA\_PLACEHOLDER\_330\_END和FORMULA\_PLACEHOLDER\_336\_END。如果这两个的值很相近呢，就让这个公式的输出的值小一点。如果不相近呢，就让它的输出大一点。”

“那我们用 对数（log） 吧！” 另一个数学家说道。

“既然不相近，我们假设这个公式的输出为FORMULA\_PLACEHOLDER\_342\_END吧！代表 **“损失”**：

如果输入的FORMULA\_PLACEHOLDER\_348\_END为 **1** ，我就让FORMULA\_PLACEHOLDER\_354\_END， 这样如果FORMULA\_PLACEHOLDER\_362\_END接近 0 时，**S**会特别大，反之会特别小。  
  如果输入的FORMULA\_PLACEHOLDER\_368\_END为 **0**， 我就让FORMULA\_PLACEHOLDER\_374\_END， 这样如果FORMULA\_PLACEHOLDER\_382\_END接近 1 时，**S**会特别大，反之会特别小。”

“那我们把它们结合一下吧，成为一个公式。同时让这两个公式添加两个非0即1的相反的乘数。虽然是一个公式，但是同时只有一半可以用。“

“我看就用 FORMULA\_PLACEHOLDER\_388\_END 和 FORMULA\_PLACEHOLDER\_394\_END 吧，这两个不是标签么？非0即1”

“太棒了！让我再来添加一个帅气的符号吧！把它变成：  
 FORMULA\_PLACEHOLDER\_400\_END”  
 “太帅了!!!,我们给它取一个名字吧！同时找一个帅气的翻译者，把它翻译词让人 **望!而!生!畏!** 的中文吧！”
...
“这好办呀！假设有FORMULA\_PLACEHOLDER\_414\_END个数据，我们把每一个单一的数据结果加起来，然后再除以FORMULA\_PLACEHOLDER\_419\_END不就好了吗？”

“既然结果依赖于FORMULA\_PLACEHOLDER\_424\_END和FORMULA\_PLACEHOLDER\_430\_END，同时FORMULA\_PLACEHOLDER\_436\_END和FORMULA\_PLACEHOLDER\_442\_END是给定的不能修改，那么可以说结果决度定于FORMULA\_PLACEHOLDER\_448\_END”

“FORMULA\_PLACEHOLDER\_454\_END不是来源于FORMULA\_PLACEHOLDER\_460\_END吗？而且刚刚不是有个高大威猛帅气的男生说 **逻辑回归的目标，就是找到合适的 FORMULA\_PLACEHOLDER\_472\_END和 FORMULA\_PLACEHOLDER\_477\_END** 吗？那我们干脆直接用 FORMULA\_PLACEHOLDER\_482\_END和 FORMULA\_PLACEHOLDER\_487\_END作表示吧，那不就是…”

FORMULA\_PLACEHOLDER\_492\_END

这，就是**损失函数（Cost Function / Loss Function）**

**在深度学习中，我们需要找合适的FORMULA\_PLACEHOLDER\_507\_END和FORMULA\_PLACEHOLDER\_512\_END，尽可能的缩小FORMULA\_PLACEHOLDER\_517\_END**
...
现在我们已经知道了“损失”，我们便可以通过“损失”的大小来反向的调整FORMULA\_PLACEHOLDER\_524\_END和FORMULA\_PLACEHOLDER\_529\_END，使得FORMULA\_PLACEHOLDER\_534\_END 画出来的决策边界可以尽可能的拟合所有的点。
...
**对于FORMULA\_PLACEHOLDER\_543\_END的更新：**  
 1：求损失函数 FORMULA\_PLACEHOLDER\_548\_END关于权重FORMULA\_PLACEHOLDER\_553\_END的偏导数  
 FORMULA\_PLACEHOLDER\_558\_END  
 2:新的FORMULA\_PLACEHOLDER\_570\_END等于当前FORMULA\_PLACEHOLDER\_575\_END倍的损失函数 FORMULA\_PLACEHOLDER\_581\_END关于权重FORMULA\_PLACEHOLDER\_586\_END的偏导数  
 FORMULA\_PLACEHOLDER\_591\_END

**对于FORMULA\_PLACEHOLDER\_601\_END的更新：**  
 1：求损失函数 FORMULA\_PLACEHOLDER\_606\_END关于权重FORMULA\_PLACEHOLDER\_611\_END的偏导数  
 FORMULA\_PLACEHOLDER\_616\_END  
 2:新的FORMULA\_PLACEHOLDER\_630\_END等于当前FORMULA\_PLACEHOLDER\_635\_END倍的损失函数 FORMULA\_PLACEHOLDER\_641\_END关于权重FORMULA\_PLACEHOLDER\_646\_END的偏导数
...
dz实际上代表的是损失函数对激活函数输出 A 的导数，在sigmoid函数中  
 FORMULA\_PLACEHOLDER\_661\_END  
 对于单个的z和b  
 FORMULA\_PLACEHOLDER\_674\_END

FORMULA\_PLACEHOLDER\_684\_END
...
1.计算 FORMULA\_PLACEHOLDER\_693\_END  
 2.选定sigmoid函数的阈值，将FORMULA\_PLACEHOLDER\_704\_END变为0（如果激活值<=0.5）或者1（如果激活值>0.5）,FORMULA\_PLACEHOLDER\_709\_END 表示神经元的激活值,FORMULA\_PLACEHOLDER\_714\_END 是sigmoid激活函数FORMULA\_PLACEHOLDER\_719\_END 的输出矩阵。
...
最后，我们得到了这样的结果和图片  
 ![在这里插入图片描述](images/blog_migrate_31f5a618d6880635ede84824b1901306_png.png)
...
![在这里插入图片描述](images/blog_migrate_1164ea1a92a6ce9e84d78b069b8f489a_png.png)
