...
**<font size="5">所谓学习：通过数据的==输入==-->得到模型的==预测值==，将==预测值==和实际的==标签==比较，标签和预测值的差异称为<font color="red">损失</font>。通过之间的损失自动调整公式里面==参数==，最终找那个可以让损失最小的、合适的公式的==参数！==</font>**

**<font size="5">不是单纯的输入数据和输入的标签学习映射关系！</font>**
...
<font size="4.5" color="blue">有没有什么办法可以让标签 (${y^i}$) 为 **1** 的 ${x^i}$ 的预测值 ($\hat{y}^i$) 趋近于 **1**？</font>

<font size="4.5" color="blue">有没有什么办法可以让标签 (${y^i}$) 为 **0** 的 ${x^i}$ 的预测值 ($\hat{y}^i$) 趋近于 **0**？</font>

这个时候，有一个拿到 **两！根！筷！子！** 的哲学家说，简单啊，你让

<font size="4.5" color="blue">可以让标签 (${y^i}$) 为 **1** 的 ${x^i}$ 的预测值 ($\hat{y}^i$) 不趋近于 **0**</font>
<font size="4.5" color="blue">可以让标签 (${y^i}$) 为 **0** 的 ${x^i}$ 的预测值 ($\hat{y}^i$) 不趋近于 **1**</font>
...
<font size="4.5" color="blue">如果输入的${y^i}$为 **1** ，我就让$Loss= -\log(\hat{y}^{(i)})$， 这样如果$\hat{y}^i$接近 0 时，**S**会特别大，反之会特别小。</font>
<font size="4.5" color="blue"> 如果输入的${y^i}$为 **0**， 我就让$Loss = -\log(1 - \hat{y}^{(i)})$， 这样如果$\hat{y}^i$接近 1 时，**S**会特别大，反之会特别小。”</font>
...
***<font size="5"  color="red">二元交叉熵损失函数（Binary Cross-Entropy Loss，也称为log loss）</font>***
...
这，就是<font size="5"  color="red">**损失函数（Cost Function / Loss Function）**</font >
