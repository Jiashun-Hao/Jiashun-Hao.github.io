...
description: "- [导言](- [前置1：神经网络的训练过程](- [前置2：代码编写过程](- [一、导入的包和说明](- [二、数据的预处理](- - [2.1 数据集说"
...
放此图在这里为了快速认识，也为了需要的时候快速查找。初次看的时候看不懂没关系，可以先看下面的内容。  
 ![在这里插入图片描述](./images/1ee86f1a2efdff2c36e7e48c935b76b2.png)
...
**举个例子：**  
 ![在这里插入图片描述](./images/0f570d28852474ad1e590a7e0f2f83e6.png)
...
![在这里插入图片描述](./images/10629ef708e6da9268d8aba0deb13c3b.png)  
 这三个底色图片的参数都不一样，因为不一样，所以才可以构成千颜万色。

对于当前的步骤，我们不需要关心每一个矩阵中的参数是怎么样的。

我们只需要知道，一个图片，在计算机中完整的**像素的个数**是：高像素 X 宽像素 X 3  
 ![在这里插入图片描述](./images/23dabf6793e16a3f3076ad70d1827afb.png)

所以，一个图片的矩阵向量的表现形式就是 **[ 高像素 , 宽像素 , 3 ]** ，这条猫猫的矩阵向量的表现形式是[5,5,3]

![在这里插入图片描述](./images/8a693d140c07b3a06a663f17045700d3.png)  
...
![在这里插入图片描述](./images/1cbd39e759001444240063c9267c3da9.png)
...
![在这里插入图片描述](./images/4329a5543ba4d75d29fb62ee6c502c37.png)
...
![在这里插入图片描述](./images/ec9298ba9c88e8836bfa6b24de76ad66.png)
...
就是说，找一条线，将数据划分为两个部分。  
 ![在这里插入图片描述](./images/fb94aafe78d02b602bc71492505bcbaf.png)

如果维度高的话，是找一个面来划分  
 ![在这里插入图片描述](./images/a225e8725234cf464766bd92c5d40493.png)  
 对于逻辑回归的函数，我们可以理解为：
...
i
y^i
yi同类型的值（这也是为什么用hat/y表示而不是用别的字母）
...
在这里，y^​i 是模型对输入 xi 的预测输出，而 yi 是实际的标签或真实值。
...
我们希望 xi 生成的预测y^​i尽可能接近 xi的标签 yi。

为了得到一个明确的分类预测（0或1），可以设定一个阈值，通常为0.5。如果 
y
^
\hat{y}
y^​大于0.5，则预测为1，否则为0。
...
w
\frac{dy}{dw}
dwdy​ 是层输出相对于权重的梯度。通过链式法则，我们可以将这两个梯度相乘，从而得到我们需要的梯度 
d
L
d
w
\frac{dL}{dw}
dwdL​。这种方法可以用来对网络中的所有权重进行类似的计算。
...
1. 前向传播：给当前数据
   ...
   i
   x^i
   xi，根据算法（当前我们用的是sigmoid函数）输出一个结果
   y
   ^
   i
   \hat y^i
...
2. 计算机损失：得到前向传播的预测
   ...
   i
   \hat y^i
   y^​i以后，我们会评估“预测与我们给定的标签
   y
   i
   y^i
   yi之间的差异。这个差异（通常称为“损失”或“误差”）
...
在把这个哲学家祭天以后，一个数学家仔细琢磨这句话，说：“不如这样吧，我们可以想一个公式，在公式里面同时输入
y
i
{y^i}
yi和
...
i
\hat{y}^i
y^​i。如果这两个的值很相近呢，就让这个公式的输出的值小一点。如果不相近呢，就让它的输出大一点。”
...
\hat{y}^(i) = \frac{1}{1 + e^{-( w^T \cdot x^{(i)} + b)}}
y^​(i)=1+e−(wT⋅x(i)+b)1​吗？而且刚刚不是有个高大威猛帅气的男生说 **逻辑回归的目标，就是找到合适的 
w
w
w和 
b
...
现在我们已经知道了“损失”，我们便可以通过“损失”的大小来反向的调整
w
w
w和
b
...
最后，我们得到了这样的结果和图片  
 ![在这里插入图片描述](./images/31f5a618d6880635ede84824b1901306.png)
...
![在这里插入图片描述](./images/1164ea1a92a6ce9e84d78b069b8f489a.png)
