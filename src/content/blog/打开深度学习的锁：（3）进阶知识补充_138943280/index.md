...
- **线性函数（Linear Functions）**：提供基本的线性函数计算。
	...
	<font size=6 color=red>$a^{[l]}_i$表示第$l$ 层的第 $i$ 个神经元的激活值</font>
...
<font size=5 >虽然大权重变少了很多，但是还是比小权重高，所以对于模型来说，**大权重的特征的重要性还是比小权重的特征的重要性要高。** </font>
...
<font size=5 >==但是对于深度网络的其它层来说，减小是上一层的输出的权重，优化的则是上一层神经网络== </font>
...
在实践中，也可以同时使用L1和L2正则化，这种组合被称为弹性网（Elastic Net）正则化。</font></font>

## 4. Dropout正则化：作用于激活值

<font size=5  color=blue> Dropout正则化不能用于测试阶段！！Dropout正则化不能用于测试阶段！！</font>
...
<font size=5 color=red>那就是 $h$, 这个移动的标量必须是一个极小值，类似$10^{-7}. 所得到的误差一般为h^2$ </font>
### 2. 梯度检查
...
<font size=5 color=red>如果 $h$ 是 $10^{-7}$ ，那么relative error的值应该在 $10^{-5}$ 到 $10^{-9}$ 之间，否则为异常。 </font>
